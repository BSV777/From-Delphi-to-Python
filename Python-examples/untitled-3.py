...
# -*- coding: utf-8 -*-
# Copyright:   (c) Andrey Stolyarov 2014
...
class MyWindow(QtGui.QWidget):   #Объявляем класс MyWindow, наследующий свойства класса QWidget модуля QtGui
    def __init__ (self, parent=None):  #Объявляем процедуру -конструктор класса MyWindow
        ...
        self.label1= QtGui.QLabel("Показатель адиабаты") # Создали объект надпись класса QLabel
        self.label2= QtGui.QLabel("Коэффициент скорости") # Создали объект надпись класса QLabel
        ...
        self.Button1 = QtGui.QPushButton("Вычислить") # Создали объект кнопка, экземпляр класса QPushButton
    ...
    def calculate(self):        #Объявление процедуры calculate - метода объекта класса MyWindow
        ...
        L=1-(k-1)/(k+1)*L**2        # Вычисляем τ
        self.plainTextEdit.appendPlainText("λ="+str(L)+",   τ="+str(T))# Записываем атрибут PlainText объекта PlainTextEdit
        ...
        self.axes.plot(x,y,label="τ(λ)") # Строим график
...
if __name__ == '__main__':  # Основная процедура программы
    ...
    window.setWindowTitle("Газодинамическая функциия τ") #Записываем атрибут WindowTitle объекта window
